...
def initialize_database():
    ...
    # if cursor.execute('SELECT name FROM sqlite_master WHERE type="table" AND name="tasks"').fetchone() is None:
    #     cursor.execute('''
    ...
    # try:
    #     cursor.execute("ALTER TABLE tasks ADD COLUMN bot TEXT DEFAULT 'None'")
    #     print('Поле bot добавлено в таблицу "tasks"')
    # except sqlite3.OperationalError as e:
    #     if "duplicate column name" in str(e):
    #         print('Выполнено подключение к полю bot в таблице "tasks".')
    #     else:
    #         print(f"Ошибка при добавлении поля bot: {e}")
